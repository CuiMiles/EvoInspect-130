...
updated_at: 2026-08-24T22:09:06+08:00
...
- 当前阶段：seed-130 的 wood/capsule/transistor/hazelnut 已完成 4/4 `metrics.json`；
  smoke-s131-132 的 8 个任务按 GPU 4--7 分两批执行，当前 4 个运行（最新 checkpoint 约
  4,800 steps）、4 个排队；尚无 `smoke-gate.json` 或正式聚合结论。GPU 0--3 由其他用户占用，
  GPU 4--7 仅有本批次进程；
