...
Updated 2026-08-24 21:47 CST. The latest completed evidence consists of a CPU localization
...
- Seed-130 wood/capsule/transistor/hazelnut completed 4/4 `metrics.json`; the smoke-s131-132 stage
  has 8 tasks total, with 4 currently running on GPUs 4--7 and 4 queued. `smoke-gate.json` is not
  present yet, so no formal smoke conclusion is available. GPUs 0--3 are currently occupied by
  another user and were not touched.
