#!/usr/bin/env python3
"""Aggregate the exploratory EfficientAD-S384 strict screen."""
...
def main() -> int:
    ...
    paths = sorted(
        args.batch_root.glob("runs/efficientad-s384-*-s143/strict_result_v2/metrics.json")
    )
    ...
    payload = {
        "schema_version": 1,
        "status": "efficientad_s384_screen_exploratory_aggregate",
        "created_at": utc_now(),
        "batch_root": str(args.batch_root.resolve()),
        "training_config": "configs/baselines/efficientad_s_384_screen_20260831.yaml",
        "evaluator_config": "configs/evaluation/efficientad_s_384_screen_20260831.yaml",
    ...
    config = yaml.safe_load(
        Path("configs/baselines/efficientad_s_384_screen_20260831.yaml").read_text(
            encoding="utf-8"
        )
    )
